...
import { ocQueryOptions } from "../lib/queries";
...
export function OperationalChangesPage() {
  ...
  const { data, isLoading, isError, error } = useQuery(
    ocQueryOptions(range, { status: statusFilter, chgCategory: categoryFilter }),
  );

  async function handleExport() {
    setExporting(true);
    await api.operationalChanges.exportCsv().catch(() => {});
    setExporting(false);
  }
    ...
    <div className="p-6 max-w-7xl mx-auto">
      ...
      {isError && (
        <div className="bg-red-50 border border-red-200 rounded-xl p-6 text-center">
          <p className="text-sm font-medium text-red-700">Failed to load operational changes</p>
        ...
        <>
          <div className="grid grid-cols-3 gap-4 mb-6">
            <div className="bg-white rounded-xl border border-gray-200 p-4">
              ...
              <p className={`text-2xl font-semibold ${data.slaRate >= 90 ? "text-green-600" : data.slaRate >= 75 ? "text-yellow-600" : "text-red-600"}`}>
          ...
          <div className="grid grid-cols-2 gap-4 mb-6">
            {data.cancelReasons.length > 0 && (
              <div className="bg-white rounded-xl border border-gray-200 p-5">
                <h2 className="text-sm font-medium text-gray-700 mb-4">Cancellation Reasons</h2>
            ...
            {data.stopReasons.length > 0 && (
              <div className="bg-white rounded-xl border border-gray-200 p-5">
                <h2 className="text-sm font-medium text-gray-700 mb-4">Stop Reasons</h2>
                <ResponsiveContainer width="100%" height={200}>
                  <BarChart data={data.stopReasons} layout="vertical">
                    <CartesianGrid strokeDasharray="3 3" horizontal={false} />
                    <XAxis type="number" tick={{ fontSize: 11 }} />
                    <YAxis dataKey="reason" type="category" width={120} tick={{ fontSize: 11 }} />
                    <Tooltip />
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-200 bg-gray-50">
                  ...
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wide">Lead Time</th>
                </tr>
              </thead>
                ...
                {data.items.length === 0 && (
                  <tr>
                    <td colSpan={6} className="px-4 py-8 text-center text-sm text-gray-400">
                      No operational changes found for the selected filters.
                    </td>
                  </tr>
                )}
                {data.items.map((item) => (
